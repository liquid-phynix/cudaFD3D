...
struct Simulation{
  ...
  Simulation(){
    ...
    if((not m_hostField) or (not m_deviceField) or (not m_deviceFieldPrime)){
      //    if((not m_hostField) or (not m_deviceField) or (not m_deviceFieldPrime) or (not m_deviceFieldLap)){
...
__global__ void kernel_fill(float* in, float* out, int sx, int sy, int sz, float ddm2){
  ...
  out[((bz + z + 3) * sy + by + y + 3) * sx + bx + x + 3] = 
    cache[x + 3][y + 3][z + 3]
    + 0.001f * (lapPsi3
  	      + (1.0f - 0.2f) * cache_lap[x + 3][y + 3][z + 3]
...
void saveFile(int fc, Simulation& sim, int z){
  char fn[SLEN];
  ...
  float buf[(sx - 6) * (sy - 6)];
  int i = 0;
  for(int y = 3; y < sy - 3; y++){
    for(int x = 3; x < sx - 3; x++){
      buf[i] = sim.m_hostField[(z * sy + y) * sx + x];
      i++;
    }
  }
  fwrite(buf, sizeof(float), (sx - 6) * (sy - 6), output);
  fclose(output);
}

void saveFileFull(int fc, Simulation& sim, int z){
  char fn[SLEN];
  genFileName(fn, "out_", fc);
  FILE* output;
  output = fopen(fn, "w");
  ...
  fwrite(buf, sizeof(float), sx * sy, output);
  fclose(output);
}
